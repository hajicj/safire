...
class MultilayerPerceptron(BaseSupervisedModel):
    ...
    def __init__(self, inputs, n_in, n_out, n_layers, n_hidden_list, 
                 ...
                 L1_w = 0.0, L2_w = 0.0,
                 hidden_layer_params=None,
                 #log_regression_params=None
        ):
        ...
        .. warning::
          ...
          Log regression layer for a non-classifier multilayer perceptron has
          been experimentally deprecated!!!
        ...
        assert n_out == n_hidden_list[-1], "Last layer size must correspond to response vector size."
        ...
        logging.debug('Hidden layer shapes: %s' % str(hidden_layer_shapes))
        #logging.debug('Log layer shape: %s' % str(log_layer_shape))
        ...
        for l_index, l_shape in enumerate(hidden_layer_shapes):
            ...
            if 'n_in' in hparams and hparams['n_in'] != l_shape[0]:
                raise ValueError('Trying to initialize multilayer perceptron with mismatched'
            ...
            hparams['n_out'] = l_shape[1]

            # Inputs are always linked to previous outputs, so they get deleted.
            if 'inputs' in hparams:
                logging.warn('Deleting inputs from hparams for layer %i; are you'
                             'really loading the model? (Where did you get the hparams?)' % l_index)
                del hparams['inputs']

            logging.debug('Initializing hidden layer with size %s.' % str(
                (hparams['n_in'], hparams['n_out'])))
        ...
        # if n_layers > 0:
        #     logging.debug('Setting log layer inputs to hidden layer outputs.')
        ...
        #     raise ValueError('Log regression layer input dimension from params (%i)'
        ...
        #     raise ValueError('Log regression layer output dimension from params (%i)'
        #                  'and arguments (%s) doesn\'t match.' % (
        ...
        #     del lparams['inputs']
        #
        # logging.debug('Initializing log regression layer with size %s.' % str(
        #     (lparams['n_in'], lparams['n_out'])))
        #
    ...
    def negative_log_likelihood(self, y):
        ...
        :returns: The negative log likelihood of the given response under
                  the model.
        """
        if not self.layers[-1].activation == TT.nnet.softmax:
            logging.warn('Negative log likelihood is not log.reg. negative log likelihood, because last layer activation is not softmax.')

        return self.layers[-1]._cost(y)
